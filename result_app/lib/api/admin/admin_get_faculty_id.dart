...
Future<List> adminGetFacultyId({@required BuildContext context}) async {
  try {
    ...
    var request = http.Request(
        'GET', Uri.parse('https://result.onrender.com/faculties-details'));
    ...
    if (status == 200) {
      ...
      if (res.containsKey("facultyId") && res.containsKey("facultyName")) {
        List<String> facultyIdArray = [];
        for (var facId in res["facultyId"]) {
          facultyIdArray.add(facId);
        }
        List<String> facultyNameArray = [];
        for (var facName in res["facultyName"]) {
          facultyNameArray.add(facName);
        }
        return [true, facultyIdArray, facultyNameArray];
      }
    } else if (status == 401) {
